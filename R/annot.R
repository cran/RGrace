...
drawDetails.annotation.old <- function(x,recording){
...
drawDetails.annotation.new <- function(x,recording){
    bullet <- function(pch=as.integer(26), size=unit(1, "char"), gp=gpar(col="black",fill="white",lty=1,lwd=1)){
    grid:::set.gpar(gp)
    .Call.graphics("L_segments",xpos,ypos-0.5*size,xpos+2*size,ypos-0.5*size,NULL,PACKAGE="grid")
    gp$lty<-1
    grid:::set.gpar(gp)
    .Call.graphics("L_points", xpos+size, ypos-0.5*size, pch, size,PACKAGE="grid")
    return(list(2*size,size))
  ...
  dead.miner <- function(){
    ...
    ypos <- ypos-unit(1,"char")
    try(.Call.graphics("L_circle",xpos+unit(lx*1,"char"),ypos+unit(lx*1,"char"),abs(unit(0.75,"char")),PACKAGE="grid"))
    try(.Call.graphics("L_segments",xpos+unit(lx*c(-0.5,-0.5,1.5,1.5),"char"),ypos+unit(ly*c(3,2,2,3),"char"),xpos+unit(lx*c(0.5,0.5,2.5,2.5),"char"),ypos+unit(ly*c(2,3,3,2),"char"),NULL,PACKAGE="grid"))
    try(.Call.graphics("L_lines",xpos+unit(lx*c(1.25,1.25,1.5,1.75,2,2),"char"),ypos+unit(ly*c(-0.325,-1,-1.75,-1.75,-1,-1),"char"),list(as.integer(1:5)),NULL,PACKAGE="grid"))
    try(.Call.graphics("L_lines",xpos+unit(lx*c(0,0.5,1,1.5,2),"char"),ypos+unit(ly*c(-1,-0.5,-0.25,-0.5,-1),"char"),list(as.integer(1:5)),NULL,PACKAGE="grid"))
...
#      .Call.graphics("L_text", xx, xpos, ypos,x$just, 0, x$check.overlap,PACKAGE="grid")
  ...
  if (x$frame) {
    .Call.graphics("L_lines",unit.c(unit(-0.5,"char"),unit(-0.5,"char"),unit(-0.5,"char")+unit(xwidth+1,"char"),unit(-0.5,"char")+unit(xwidth+1,"char"),unit(-0.5,"char")),unit.c(unit(0.5,"char"),unit(0.5,"char")+unit(-heighta-1,"char"),unit(0.5,"char")+unit(-heighta-1,"char"),unit(0.5,"char"),unit(0.5,"char")),list(as.integer(1:5)),NULL,PACKAGE="grid")
  }
}
